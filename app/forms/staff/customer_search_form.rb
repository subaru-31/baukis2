class Staff::CustomerSearchForm
  ...
  private
    def normalize_values
      ...
      self.city = normalize_as_furigana(city)
      self.phone_number = normalize_as_phone_number(phone_number).try(:gsub, /\D/, "")
      self.last_four_digits_of_phone_number = normalize_as_phone_number(last_four_digits_of_phone_number).try(:gsub, /\D/, "")
      self.postal_code = normalize_as_postal_code(postal_code).try(:gsub, /\D/, "")
      Rails.logger.debug(postal_code)
    end
end